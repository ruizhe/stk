...
On macOS, browser commands located inside an `.app` bundle are opened through `NSWorkspace` and LaunchServices. STK passes the existing browser arguments and resolved environment through `NSWorkspaceOpenConfiguration`, so managed profiles, proxy settings, private mode, and custom arguments keep their current behavior without directly executing another application's internal Mach-O binary. Non-bundle commands, browser entries that require a working directory, and ordinary application launchers retain direct process execution.
...
STK does not impose a fixed active-connection ceiling on listeners. When no healthy SSH session is available, an accepted connection remains pending and wakes on session or network recovery events instead of failing because an application-level recovery deadline expired. Established channels remain on their original session while replacement capacity is prepared. A transient listener `accept` error also keeps the existing socket bound and retries in place, avoiding a rebind window that could refuse new TCP connections.
...
The runtime subscribes to system interface and route change events on macOS, Linux, and Windows. While the system is confirmed offline it creates no new SSH session tasks and performs no replacement, scale-up, scheduled rotation, or remote-owner recovery. Custom health probes pause failure accounting, while existing TCP/SSH sessions are left untouched so a connection that survives a brief interruption can continue. Resume notifications are edge-triggered and unchanged network snapshots are not rebroadcast. After connectivity returns or the machine resumes from sleep, the runtime refreshes network state and observes a short settling deadline before restoring pool capacity and remote listeners; session exits and incoming demand cannot cancel that deadline early. This prevents cross-host refresh amplification and wake-time connection bursts.
