...
When a probe approaches the failure threshold, the runtime still requests a replacement, immediately stops assigning new channels to the suspect session, and drains its existing channels after the target pool capacity has recovered. Health replacement preserves the current target pool size and remains subject to connection cooldown, exponential backoff, and at most two concurrent SSH handshakes per host, preventing reconnect storms on poor networks.
...
To limit battery impact on mobile systems, session capacity management runs once per second, requires sustained high utilization before scaling up, adds only one elastic session at a time, and uses a long idle window before scaling down. Every baseline connection in `min-sessions` still performs keepalive and health-probe work, so larger baseline values inherently increase background network activity and power use. This pool-management logic lives in the cross-platform core and behaves the same on macOS, Linux, and Windows.
