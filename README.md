...
When opening a channel fails, one proxy dial tries at most three healthy sessions. This bounds request latency and resource retention during a destination or route outage instead of multiplying the per-session channel timeout by every session in a large pool.
